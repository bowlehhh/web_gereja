...
<form action="{{ route('admin.cabang.update', $cabang) }}" method="POST" enctype="multipart/form-data"
  ...
  <div class="p-6 sm:p-8 space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        <label class="font-extrabold text-sm text-blue-900">Email <span class="text-slate-500">(Opsional)</span></label>
        <input type="email" name="email" value="{{ old('email', $cabang->email) }}"
               class="mt-2 w-full h-11 px-4 rounded-xl border border-blue-200 text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:border-transparent transition">
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <label class="font-extrabold text-sm text-blue-900">Latitude Map <span class="text-slate-500">(Opsional)</span></label>
        <input type="number" step="any" name="map_latitude" value="{{ old('map_latitude', $cabang->map_latitude) }}"
               placeholder="-0.5021836"
               class="mt-2 w-full h-11 px-4 rounded-xl border border-blue-200 text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:border-transparent transition">
      </div>

      <div>
        <label class="font-extrabold text-sm text-blue-900">Longitude Map <span class="text-slate-500">(Opsional)</span></label>
        <input type="number" step="any" name="map_longitude" value="{{ old('map_longitude', $cabang->map_longitude) }}"
               placeholder="117.1537097"
               class="mt-2 w-full h-11 px-4 rounded-xl border border-blue-200 text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:border-transparent transition">
      </div>
    </div>
    <p class="-mt-3 text-xs font-semibold text-slate-500">Opsional. Isi keduanya agar map tampil di halaman detail cabang. Klik kanan titik di Google Maps untuk copy koordinat.</p>
